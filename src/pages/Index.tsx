...
export default function Index() {
  ...
  const handleModelUpload = useCallback(async (file: File | null) => {
    ...
    if (file) {
      ...
      setModelBundle({
        features: [
          'age', 'stage_ordinal', 'tumor_size', 'lymph_nodes',
          'EGFR_pTPM', 'TP53_pTPM', 'KRAS_pTPM', 'ALK_pTPM',
          'BRCA1_pTPM', 'BRCA2_pTPM', 'MYC_pTPM', 'HER2_pTPM'
        ],
        riskRef: { q33: 0.35, q66: 0.65 },
        featureCount: 12
      });
  ...
  const handlePredict = useCallback(async () => {
    ...
    try {
      ...
      const predictions: PatientResult[] = patientData.map((row, idx) => {
        ...
        // Classify risk
        const { group: riskGroup } = classifyRisk(riskScore, modelBundle.riskRef || null);
      ...
      <div className="relative z-10 min-h-screen">
        ...
        <main className="container mx-auto px-6 py-8">
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-8">
            <FileUploader
              accept=".joblib"
              label="Model Bundle"
              description="Upload .joblib file with trained RSF model"
              icon="model"
              file={modelFile}
            ...
            <FileUploader
              accept=".xlsx,.xls,.csv"
              label="Patient Data"
              description="Upload .xlsx or .csv with patient features"
              icon="data"
              file={dataFile}
              ...
              <div className="flex flex-wrap items-center justify-between gap-6">
                <div className="flex items-center gap-6">
                  <div className="flex items-center gap-3">
                    <Switch
                      id="raw-ptpm"
                      checked={rawPtpmInput}
                      onCheckedChange={setRawPtpmInput}
                    />
                      ...
                      {TIMEPOINT_OPTIONS.map(year => (
                        <button
                          ...
                          className={`px-2.5 py-1 text-xs rounded-md transition-colors ${
                            selectedTimepoints.includes(year)
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Button
                    ...
                    disabled={patientData.length === 0 || isProcessing}
                    className="gap-2"
                  >
          ...
          {/* Data Editor */}
          {patientData.length > 0 && columns.length > 0 && (
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              className="mb-8"
            ...
            {results.length > 0 && summaryStats && (
              ...
              >
                ...
                <div className="mb-8">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <MetricCard
                      ...
                      value={summaryStats.avgRisk.toFixed(4)}
                      icon={Activity}
                    />
                    <MetricCard
                      label="Min Risk"
                      value={summaryStats.minRisk.toFixed(4)}
                      icon={TrendingDown}
                      variant="low"
                    />
                    <MetricCard
                      label="Max Risk"
                      value={summaryStats.maxRisk.toFixed(4)}
                      icon={TrendingUp}
                      variant="high"
                    />
                  </div>
                ...
                <div className="mb-8">
                  ...
                  {chartMode === 'single' && results.length > 0 && (
                    <div className="mb-4">
                      <Select
                        value={selectedPatient || results[0].patientId}
                  ...
                  <SurvivalChart
                    mode={chartMode}
                    data={chartData}
                    timepoints={selectedTimepoints.map(y => y * 365)}
                    selectedPatient={selectedPatient || undefined}
                  />
                  
                  {/* Selected Patient Details */}
                  {chartMode === 'single' && selectedPatientData && (
                    <motion.div
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                      className="mt-6"
                    >
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
                        <MetricCard
                          label="Patient"
                          value={selectedPatientData.patientId}
                        />
                        <MetricCard
                          label="Risk Score"
                          value={selectedPatientData.riskScore.toFixed(4)}
                        />
                        <MetricCard
                          label="Risk Group"
                          value={selectedPatientData.riskGroup || '—'}
                          variant={
                            selectedPatientData.riskGroup === 'Low' ? 'low' :
                            selectedPatientData.riskGroup === 'Intermediate' ? 'medium' :
                            selectedPatientData.riskGroup === 'High' ? 'high' : 'default'
                          }
                        />
                      </div>
                      ...
                      <div className="glass-card rounded-xl p-5">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                          {selectedPatientData.survivalProbabilities.map(sp => {
                            const variant = sp.probability >= 0.7 ? 'low' : 
                                           sp.probability >= 0.4 ? 'medium' : 'high';
                            return (
                              <MetricCard
                                key={sp.year}
                                label={`Year ${sp.year}`}
                                value={`${(sp.probability * 100).toFixed(1)}%`}
                                variant={variant}
                              />
                            );
                          })}
        ...
        <footer className="border-t border-border/50 mt-16">
          <div className="container mx-auto px-6 py-6 text-center">
            <p className="text-sm text-muted-foreground">
              RSF Survival Predictor • Advanced Machine Learning for Personalized Survival Analysis
            </p>
          </div>
        </footer>
      </div>
